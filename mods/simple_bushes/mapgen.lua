minetest.register_on_generated(function(minp, maxp, seed)
	if maxp.y >= 2 and minp.y <= 0 then
		-- Generate flowers
		local perlin1 = minetest.get_perlin(534, 3, 0.6, 100)
				...
				if ground_y then
					...
					if minetest.registered_nodes[nn] and
						...
						if nn == "default:dirt_with_grass" then
							local flower_choice = pr:next(1, 17)
							local flower
							if flower_choice == 1 then
								flower = "simple_bushes:orange"
							elseif flower_choice == 2 then
								flower = "simple_bushes:red"
							elseif flower_choice == 3 then
								flower = "simple_bushes:yellow"
							elseif flower_choice == 4 then
								flower = "simple_bushes:white"
							elseif flower_choice == 5 then
								flower = "simple_bushes:blue"
							elseif flower_choice == 6 then
								flower = "simple_bushes:violet"
							elseif flower_choice >= 7 then
								flower = "simple_bushes:green"
							end
