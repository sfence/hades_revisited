...
if minetest.get_modpath("moretrees") ~= nil then
	...
	overwrite("moretrees:acorn_muffin", 5)
...
if minetest.get_modpath("bushes") ~= nil then
	...
	overwrite("bushes:berry_pie_cooked", 5)
...
if minetest.get_modpath("farming_plus") ~= nil then
	overwrite("farming_plus:carrot_item", 2)
	...
	overwrite("farming_plus:potato_item", 0)
	overwrite("farming_plus:rhubarb_item", 2)
	overwrite("farming_plus:olive", 1)
end
...
if minetest.get_modpath("cofo") ~= nil then

                overwrite("cofo:seagrass", 1)
	
                overwrite("cofo:chocolate", 2)

                overwrite("cofo:chocobanana", 4)

                overwrite("cofo:apple_danish", 5)

                overwrite("cofo:carrangajuice", 5)

                overwrite("cofo:hotpot_vegi_u", 3)

                overwrite("cofo:hotpot_vegi", 8)

                 overwrite("cofo:hotpot", 10)

                overwrite("cofo:milkbottle", 2, "vessels:glass_bottle")
                overwrite("cofo:honeymilk", 6, "vessels:drinking_glass")


                overwrite("cofo:cooked_egg", 5)


                overwrite("cofo:rhubarb_pie_raw", 3)

                overwrite("cofo:rhubarb_pie", 8)

                overwrite("cofo:strawberry_pie_raw", 3)

                overwrite("cofo:strawberry_pie", 8)

                overwrite("cofo:sugar", 1)

                overwrite("cofo:pumpkin_egg", 8)
