...
zcg.formspec = function(pn)
	...
	for _, name in ipairs(zcg.itemlist) do
		if s < page*npp then s = s+1 else
			...
			formspec = formspec ..default.gui_bg_img..default.gui_slots.. "item_image_button["..(i%8)..","..(math.floor(i/8)+3.5)..";1,1;"..name..";zcg:"..name..";]"
			i = i+1
		end
	end
	if page > 0 then
		formspec = formspec .. "button[0,2;1,2.55;zcg_page:"..(page-1)..";<<]"
	end
	if i >= npp then
		formspec = formspec .. "button[1,2;1,2.55;zcg_page:"..(page+1)..";>>]"
	end
	formspec = formspec .. "label[0,2.35;Page "..(page+1).."/"..(math.floor(#zcg.itemlist/npp+1)).."]"	-- The Y is approximatively the good one to have it centered vertically...
	return formspec
end
