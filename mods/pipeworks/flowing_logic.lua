...
pipeworks.spigot_check = function(pos, node)
	...
	if belowname and (belowname == "air" or belowname == "hades_core:water_flowing" or belowname == "hades_core:water_source") then 
		...
		if near_node and string.find(near_node.name, "_loaded") then
			if spigotname and spigotname == "pipeworks:spigot" then
				...
				if finitewater or belowname ~= "hades_core:water_source" then
					minetest.add_node({x=pos.x,y=pos.y-1,z=pos.z},{name = "hades_core:water_source"})
				end
			end
		else
			if spigotname == "pipeworks:spigot_pouring" then
				minetest.add_node({x=pos.x,y=pos.y,z=pos.z},{name = "pipeworks:spigot", param2 = fdir})
				if belowname == "hades_core:water_source" and not finitewater then
...
pipeworks.fountainhead_check = function(pos, node)
	...
	if abovename and (abovename == "air" or abovename == "hades_core:water_flowing" or abovename == "hades_core:water_source") then 
		...
		if near_node and string.find(near_node.name, "_loaded") then
			if fountainhead_name and fountainhead_name == "pipeworks:fountainhead" then
				...
				if finitewater or abovename ~= "hades_core:water_source" then
					minetest.add_node({x=pos.x,y=pos.y+1,z=pos.z},{name = "hades_core:water_source"})
				end
			end
		else
...
if abovename == "hades_core:water_source" and not finitewater then
